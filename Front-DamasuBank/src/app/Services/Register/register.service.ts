...
export class RegisterService {
  ...
  registerUser(name : string , age : number , id: number,  phoneNumber:string, address:string , email: string, password:string): Observable<any>{  
    return this.http.post<any>("http://3.140.1.8:8080/users" , { name: name , age:age , id:id, phoneNumber:phoneNumber , address:address , email:email , password:password})
  }

  registerUserDataBase(id: number): Observable<any>{  
    return this.http.post<any>("http://3.140.1.8:8080/balance" , {id:id})
  }

  registerUserRecords(id: number): Observable<any>{  
    return this.http.post<any>("http://3.140.1.8:8080/records" , {id:id})
  }
  ...
  registerUserCDT( name:string ,id: number, BeginDate: string, PeriodMonths: string, investedMoney:number, ): Observable<any>{ 
    ...
    return this.http.post<any>("http://3.140.1.8:8080/cdt" ,{name:name,id:id,BeginDate:BeginDate,PeriodMonths:PeriodMonths,investedMoney:investedMoney}, {headers})
  }
