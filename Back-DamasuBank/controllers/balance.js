...
const usuarioCapitalGet = async (req,res) =>{
    try{
        const info = await userCapitalDamasuSchema.find()
...
const usuarioCapitalGetSpecific = async (req,res) =>{
    try{
        const data = req.user
        const user = await userCapitalDamasuSchema.findOne({ "id": data.id})
...
const usuarioCapitalPostRegistro = async (req,res) => {
    try{
        const data = req.body 
        const result = await userCapitalDamasuSchema.create(data)
...
const usuarioCapitalUpdate = async (req, res) =>{
    try{
        ...
        for (let i = 0; i <= updatedInfo2.length; i++) {
            const decompressedObject2  = updatedInfo2[i];
                ...
                if (i === updatedInfo2.length) {
                    ...
                    const finalinfo1 = {
                        "name": name2 , "date" : formattedDateTime, "amount" : "+" + info.request, "id": i,
                    }  
...
const usuarioCapitalDelete = async (req,res) =>{ 
    try{
        const idforDelete = req.params.id
        await userCapitalDamasuSchema.findByIdAndDelete(idforDelete)
...
export {usuarioCapitalGet, usuarioCapitalGetSpecific ,usuarioCapitalPostRegistro, usuarioCapitalUpdate , usuarioCapitalDelete}